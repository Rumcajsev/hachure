...
// ── Polygon boolean difference (Greiner-Hormann) ─────────────────────────────

type GHVertex = {
  x: number; y: number
  next: GHVertex; prev: GHVertex
  neighbour: GHVertex | null
  entry: boolean   // true = entry into clip, false = exit
  alpha: number    // interpolation param along edge (0–1)
  intersect: boolean
  checked: boolean
}

function ghInsert(start: GHVertex, node: GHVertex): void {
  let cur = start
  while (cur.next !== start && cur.next.alpha < node.alpha) cur = cur.next
  node.next = cur.next
  node.prev = cur
  cur.next.prev = node
  cur.next = node
...
function ghBuild(pts: [number, number][]): GHVertex {
  const verts: GHVertex[] = pts.map(([x, y]) => ({
    x, y, next: null!, prev: null!, neighbour: null,
    entry: false, alpha: 0, intersect: false, checked: false,
  }))
...
function segIntersectGH(
  p1x: number, p1y: number, p2x: number, p2y: number,
  p3x: number, p3y: number, p4x: number, p4y: number,
): { alpha: number; beta: number; x: number; y: number } | null {
  const d1x = p2x - p1x, d1y = p2y - p1y
  const d2x = p4x - p3x, d2y = p4y - p3y
  const denom = d1x * d2y - d1y * d2x
  if (Math.abs(denom) < 1e-10) return null
  const dx = p3x - p1x, dy = p3y - p1y
  const alpha = (dx * d2y - dy * d2x) / denom
  const beta  = (dx * d1y - dy * d1x) / denom
  if (alpha < 0 || alpha > 1 || beta < 0 || beta > 1) return null
  return { alpha, beta, x: p1x + alpha * d1x, y: p1y + alpha * d1y }
}

function ghCollect(start: GHVertex): [number, number][] {
  const pts: [number, number][] = []
  let cur = start
  do { pts.push([cur.x, cur.y]); cur = cur.next } while (cur !== start)
  return pts
}

/** Subtract clipPoly from subjectPoly. Returns resulting polygons (0, 1, or 2+).
 *  Uses simplified Greiner-Hormann for the difference operation. */
export function subtractPolygon(
  ...
  const subHead = ghBuild(subject)
  const clipHead = ghBuild(clip)

  // Phase 1: find intersections and insert into both lists
  let foundAny = false
  let sEdge = subHead
  do {
    let cEdge = clipHead
    const intersectsOnEdge: { node: GHVertex; alpha: number }[] = []
    do {
      const r = segIntersectGH(sEdge.x, sEdge.y, sEdge.next.x, sEdge.next.y,
                               cEdge.x, cEdge.y, cEdge.next.x, cEdge.next.y)
      if (r) {
        const sNode: GHVertex = { x: r.x, y: r.y, next: null!, prev: null!, neighbour: null, entry: false, alpha: r.alpha, intersect: true, checked: false }
        const cNode: GHVertex = { x: r.x, y: r.y, next: null!, prev: null!, neighbour: null, entry: false, alpha: r.beta,  intersect: true, checked: false }
        sNode.neighbour = cNode; cNode.neighbour = sNode
        intersectsOnEdge.push({ node: sNode, alpha: r.alpha })
        ghInsert(cEdge, cNode)
        foundAny = true
      }
      cEdge = cEdge.next
    } while (cEdge !== clipHead)
    // Insert subject intersections sorted by alpha
    intersectsOnEdge.sort((a, b) => a.alpha - b.alpha)
    for (const { node } of intersectsOnEdge) ghInsert(sEdge, node)
    sEdge = sEdge.next
    while (sEdge.intersect) sEdge = sEdge.next
  } while (sEdge !== subHead)

  if (!foundAny) {
    // No intersections — subject fully inside or outside clip
    const firstInside = pointInPolygon(subHead.x, subHead.y, clip)
    return firstInside ? [] : [subject]
  }

  // Phase 2: mark entries/exits on subject list
  // For DIFFERENCE: entry = subject enters clip (outside→inside), exit = subject exits clip
  let isInside = pointInPolygon(subHead.x, subHead.y, clip)
  let cur = subHead
  do {
    if (cur.intersect) { cur.entry = !isInside; isInside = !isInside }
    cur = cur.next
  } while (cur !== subHead)

  // Phase 3: collect result polygons
  // For difference, we start outside and when we hit an ENTRY, jump to clip
  // and walk clip BACKWARD to the matching exit, then return to subject.
  const results: [number, number][][] = []
  cur = subHead
  do {
    if (cur.intersect && cur.entry && !cur.checked) {
      ...
      do {
        ...
        if (!v.intersect) {
          ...
          // Entry: add point, switch to clip going backward
          poly.push([v.x, v.y])
          v = v.neighbour!
          v.checked = true
          do {
            v = v.prev  // go backward on clip (traces hole boundary correctly for difference)
            v.checked = true
            poly.push([v.x, v.y])
          } while (!v.intersect)
          // Now v is the exit intersection on clip — jump back to subject
          v = v.neighbour!
          v.checked = true
        }
      } while (!v.checked || v !== cur)
      if (poly.length >= 3) results.push(poly)
    }
    cur = cur.next
  } while (cur !== subHead)

  // Also collect any subject regions that were never entered (fully outside clip)
  if (results.length === 0) return [subject]
  return results
}
