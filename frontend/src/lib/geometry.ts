...
// ── Polygon split (corridor subtraction) ─────────────────────────────────────

function segIsect2(
  p1x: number, p1y: number, p2x: number, p2y: number,
  p3x: number, p3y: number, p4x: number, p4y: number,
): { alpha: number; x: number; y: number } | null {
  ...
  return { alpha, x: p1x + alpha * d1x, y: p1y + alpha * d1y }
}

/** Split subject polygon by clip polygon, keeping only arcs that are outside the clip.
 *  For a corridor cutting through a blob, this produces two arc polygons (one on each side).
 *  Each arc closes with a straight edge — shapeTerrainBlobs makes that edge organic.
 *  Falls back to [subject] if clip doesn't intersect subject. */
export function subtractPolygon(
  subject: [number, number][],
  clip: [number, number][],
): [number, number][][] {
  const n = subject.length, m = clip.length
  if (n < 3 || m < 3) return [subject]

  type Hit = { pt: [number, number]; edge: number; t: number }
  const hits: Hit[] = []

  for (let i = 0; i < n; i++) {
    const s0 = subject[i], s1 = subject[(i + 1) % n]
    for (let j = 0; j < m; j++) {
      const c0 = clip[j], c1 = clip[(j + 1) % m]
      const r = segIsect2(s0[0], s0[1], s1[0], s1[1], c0[0], c0[1], c1[0], c1[1])
      if (r) hits.push({ pt: [r.x, r.y], edge: i, t: r.alpha })
    }
  }

  if (hits.length < 2) {
    return pointInPolygon(subject[0][0], subject[0][1], clip) ? [] : [subject]
  }

  hits.sort((a, b) => a.edge !== b.edge ? a.edge - b.edge : a.t - b.t)

  // Deduplicate hits that are nearly coincident
  const deduped: Hit[] = [hits[0]]
  for (let k = 1; k < hits.length; k++) {
    const prev = deduped[deduped.length - 1], cur = hits[k]
    if (prev.edge === cur.edge && Math.abs(prev.t - cur.t) < 1e-6) continue
    deduped.push(cur)
  }
  if (deduped.length < 2) return [subject]

  // For each adjacent pair of hits (wrapping), check if the arc between them
  // is OUTSIDE the clip by sampling the first subject vertex in that arc.
  // Outside arcs are kept as result polygons.
  const results: [number, number][][] = []
  const nh = deduped.length

  for (let k = 0; k < nh; k++) {
    const from = deduped[k]
    const to   = deduped[(k + 1) % nh]

    // Sample the first subject vertex after 'from' to decide inside/outside
    const firstVertexEdge = (from.edge + 1) % n
    const sample = subject[firstVertexEdge]
    const isOutside = !pointInPolygon(sample[0], sample[1], clip)

    // Handle degenerate case: from and to are on same edge (no vertices between)
    // Use midpoint of from.pt and to.pt as sample
    const sameEdgeArc = (from.edge === to.edge && from.t < to.t) ||
                        (from.edge === (to.edge - 1 + n) % n)
    const effectivelyOutside = sameEdgeArc
      ? !pointInPolygon((from.pt[0] + to.pt[0]) / 2, (from.pt[1] + to.pt[1]) / 2, clip)
      : isOutside

    if (!effectivelyOutside) continue

    // Build the arc: from.pt → subject vertices → to.pt
  ...
  return results.length > 0 ? results : [subject]
}
