import type { MapStore, RawRoadWay, RoadEdge, HexRoadPath, RoadTierStyle, RoadGeomOverride, ActiveTool, RoadImageSwatch, ExtractedRoadWay } from '../mapStore'
...
export type RoadsSlice = {
  ...
  setRoadCenterPull: (v: number) => void
  ...
  // ── Image-based road extraction (color-trace roads out of the aligned map image) ──
  roadImageSwatches: RoadImageSwatch[]
  extractedRoadWays: ExtractedRoadWay[]
  roadImageExtractStatus: 'idle' | 'extracting' | 'done' | 'error'
...
export const createRoadsSlice = (set: Set, get: () => MapStore): RoadsSlice => ({
  ...
  roadImageExtractPreviewOpen: false,
  setRoadImageExtractPreviewOpen: (open) => set({ roadImageExtractPreviewOpen: open }),
  ...
  fetchRoadType: async (highway) => {
    const { generatedMetadata, hexOrientation } = get()
    if (!generatedMetadata) return
    set(s => ({ roadTypeFetchStatus: { ...s.roadTypeFetchStatus, [highway]: 'loading' } }))
    try {
      ...
      set(s => ({
        rawRoadWays: [...s.rawRoadWays.filter(w => w.highway !== highway), ...(data.raw_ways as RawRoadWay[])],
        osmHexPaths: [...s.osmHexPaths.filter(p => p.highway !== highway), ...(data.hex_paths as HexRoadPath[])],
        roadTypeFetchStatus: { ...s.roadTypeFetchStatus, [highway]: 'done' },
      }))
    } catch (e) {
      set(s => ({ roadTypeFetchStatus: { ...s.roadTypeFetchStatus, [highway]: 'error' } }))
    }
  },

  applyRoadType: (highway) => {
    get().pushUndoSnapshot()
    const tier = (HIGHWAY_TO_TIER[highway] ?? 2) as 0 | 1 | 2
    const { osmHexPaths, roadEdges } = get()
    const existingPairs = new Set<string>()
    for (const e of roadEdges) {
      const a = `${e.q1},${e.r1}`, b = `${e.q2},${e.r2}`
    ...
    const newEdges: RoadEdge[] = []
    for (const path of osmHexPaths) {
      if (path.highway !== highway) continue
      for (let i = 0; i < path.hexes.length - 1; i++) {
        const [q1, r1] = path.hexes[i]
        const [q2, r2] = path.hexes[i + 1]
        const dq = q2 - q1, dr = r2 - r1
  ...
  setRoadCenterPull: (v) => set({ roadCenterPull: v }),
  ...
  batchAddRoadEdges: (edges) => {
    ...
    get().pushUndoSnapshot()
    const pk = (q1: number, r1: number, q2: number, r2: number) => {
      const a = `${q1},${r1}`, b = `${q2},${r2}`; return a < b ? `${a}|${b}` : `${b}|${a}`
    }
    // Last entry wins per pair key (handles zigzag duplicates in stroke buffer)
    const patchMap = new Map<string, { q1: number; r1: number; q2: number; r2: number; tier: 0 | 1 | 2 }>()
    for (const e of edges) patchMap.set(pk(e.q1, e.r1, e.q2, e.r2), e)
    const kept = get().roadEdges.filter(e => !patchMap.has(pk(e.q1, e.r1, e.q2, e.r2)))
    const added = [...patchMap.values()].map(e => ({ q1: e.q1, r1: e.r1, q2: e.q2, r2: e.r2, tier: e.tier, manual: true as const }))
    set({ roadEdges: [...kept, ...added] })
  },
  batchRemoveRoadEdges: (edges) => {
    ...
    const removeKeys = new Set(edges.map(({ q1, r1, q2, r2 }) => pk(q1, r1, q2, r2)))
    set({ roadEdges: get().roadEdges.filter(e => !removeKeys.has(pk(e.q1, e.r1, e.q2, e.r2))) })
  },
  ...
  addRoadImageSwatch: (color, tolerance = 40, tier = null) => {
    const { roadImageSwatches } = get()
    const id = roadImageSwatches.length ? Math.max(...roadImageSwatches.map(s => s.id)) + 1 : 0
    set({ roadImageSwatches: [...roadImageSwatches, { id, color, tolerance, tier }] })
    return id
  },
  updateRoadImageSwatchTolerance: (id, tolerance) => set(s => ({
    roadImageSwatches: s.roadImageSwatches.map(sw => sw.id === id ? { ...sw, tolerance } : sw),
  })),
  removeRoadImageSwatch: (id) => set(s => ({
    ...
    try {
      ...
      for (const line of traced) {
        ...
        for (const [u, v] of line.points) {
          const pt = imagePixelToCanvasPoint(u, v, mapImageTransform, imgData.width, imgData.height, rect.px, rect.py, rect.pw, rect.ph)
          if (!pt) continue
          const [lon, lat] = unprojectFromCanvas(pt[0], pt[1], generatedMetadata, rect.pw, rect.ph, rect.px, rect.py)
          coords.push([lon, lat])
        }
  ...
  applyExtractedRoadTier: (tier) => {
    const { extractedRoadWays, generatedMetadata, hexOrientation, roadEdges } = get()
    if (!generatedMetadata) return
    const ways = extractedRoadWays.filter(w => w.tier === tier)
    if (ways.length === 0) return
    get().pushUndoSnapshot()
    const seen = new Set<string>()
    const newEdges: RoadEdge[] = []
    for (const way of ways) {
      const hexPath = snapWayToHexPath(way.coords, generatedMetadata, hexOrientation)
      const edges = hexPathToRoadEdges(hexPath, tier, [...roadEdges, ...newEdges])
