...
export interface HexDisableRefs {
  metaRef: MutableRefObject<GridMetadata | null>
  hexesRef: MutableRefObject<GeneratedHex[]>
  activeToolRef: MutableRefObject<{ type: string; mode?: 'disable' | 'enable' }>
  toggleDisabledHexRef: MutableRefObject<(key: string, mode: 'disable' | 'enable') => void>
  clientToLogical: LogicalFn
  getPaper: GetPaperFn
}

export function attachHexDisableHandlers(el: HTMLElement, refs: HexDisableRefs): () => void {
  const { metaRef, hexesRef, activeToolRef, toggleDisabledHexRef, clientToLogical, getPaper } = refs

  const paintAt = (clientX: number, clientY: number, mode: 'disable' | 'enable', lastKey: { v: string | null }) => {
    ...
    for (const hex of hexesRef.current) {
      ...
      if (pointInPolygon(lx, ly, verts)) {
        ...
        if (key !== lastKey.v) { lastKey.v = key; toggleDisabledHexRef.current(key, mode) }
        break
      }
    }
  }
  ...
  const onDown = (e: MouseEvent) => {
    ...
    active = true
    mode = (tool as { type: string; mode: 'disable' | 'enable' }).mode
    lastKey.v = null
